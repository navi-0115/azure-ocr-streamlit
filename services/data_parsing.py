...
def parse_invoice_data(text):
    """
    Parses invoice data and structures it to fit the Invoice and InvoiceItems models.

    Args:
        text (str): The raw extracted text from an invoice.

    Returns:
        dict: A dictionary containing structured data for the invoice and its items.
    """
    ...
    parsed_data = {
        "invoice": {},  # Data for the Invoice model
        "invoice_items": []  # Data for the InvoiceItems model
    }

    # Parse fields for the Invoice model
    for field, pattern in patterns.items():
        match = re.search(pattern, text)
        if match:
            if field == "issue_date":
                raw_date = match.group(0).strip()
                parsed_data["invoice"][field] = convert_date_format(raw_date)
            elif field == "invoice_type":
                invoice_type = match.group(1).strip()
                parsed_data["invoice"][field] = invoice_type
            else:
                parsed_data["invoice"][field] = match.group(1).replace(",", "").strip()
    ...
    for item_match in re.finditer(item_pattern, text):
        ...
        parsed_data["invoice_items"].append(item_data)
...
#     parsed_data = {
#         "invoice": {},  # Data for the Invoice model
#         "items": []     # Data for the InvoiceItems model
#     }
